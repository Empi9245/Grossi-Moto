...
function CornerMask({ className, path }: { className: string; path: string }) {
  return (
    <svg
      aria-hidden="true"
      className={className}
...
export function BottomRightCorner() {
  ...
  return (
    ...
    >
      ...
      >
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[0.8rem] bg-[oklch(92%_0.014_78)] text-[oklch(17%_0.014_56)] transition-colors duration-200 group-hover:bg-[oklch(96%_0.01_80)] sm:h-11 sm:w-11">
          <ActionMark compact />
        </span>
        <span className="sr-only min-w-0 sm:not-sr-only">
          <span className="block text-xs font-semibold leading-normal sm:text-base sm:leading-[1.25]">
            Showroom e officina
          </span>
          <span className="mt-1 block text-[0.68rem] leading-normal text-[oklch(86%_0.012_78/0.72)] sm:text-sm sm:leading-[1.43]">
            Via Festo Porzio, 22
          </span>
        </span>
      </a>

...
export function MobileAppNav() {
  ...
  return (
    <nav
      ...
      inert={!isVisible}
      className={`mobile-app-nav fixed inset-x-3 bottom-[calc(0.75rem+env(safe-area-inset-bottom))] z-[70] grid grid-cols-4 gap-1 overflow-hidden rounded-[1.45rem] bg-[oklch(12%_0.014_42/0.94)] p-1.5 text-[oklch(92%_0.012_78)] shadow-[0_16px_42px_rgba(17,11,9,0.28)] ring-1 ring-[oklch(96%_0.008_80/0.12)] backdrop-blur-xl transition-[clip-path] duration-[340ms] ease-[cubic-bezier(0.22,1,0.36,1)] will-change-[clip-path] [&>a]:transition-opacity [&>a]:duration-150 [&>a]:ease-out motion-reduce:transition-none motion-reduce:[&>a]:transition-none lg:hidden ${
        isVisible
          ? "[clip-path:inset(0_0_0_0_round_1.45rem)] [&>a]:delay-[75ms] [&>a]:opacity-100"
          : "pointer-events-none [clip-path:inset(0_50%_0_50%_round_999px)] [&>a]:delay-0 [&>a]:opacity-0"
      }`}
    >
